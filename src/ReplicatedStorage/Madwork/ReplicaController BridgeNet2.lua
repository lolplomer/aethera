...
local BridgeNet2 = require(game.ReplicatedStorage:WaitForChild('Packages'):WaitForChild("bridgenet2"))
local rev_ReplicaRequestData = BridgeNet2.ReferenceBridge("Replica_ReplicaRequestData")   -- Fired client-side when the client loads for the first time

local rev_ReplicaSetValue = BridgeNet2.ReferenceBridge("Replica_ReplicaSetValue")         -- (replica_id, {path}, value)
local rev_ReplicaSetValues = BridgeNet2.ReferenceBridge("Replica_ReplicaSetValues")       -- (replica_id, {path}, {values})
local rev_ReplicaArrayInsert = BridgeNet2.ReferenceBridge("Replica_ReplicaArrayInsert")   -- (replica_id, {path}, value)
local rev_ReplicaArraySet = BridgeNet2.ReferenceBridge("Replica_ReplicaArraySet")         -- (replica_id, {path}, index, value)
local rev_ReplicaArrayRemove = BridgeNet2.ReferenceBridge("Replica_ReplicaArrayRemove")   -- (replica_id, {path}, index)
local rev_ReplicaWrite = BridgeNet2.ReferenceBridge("Replica_ReplicaWrite")               -- (replica_id, func_id, params...)
local rev_ReplicaSignal = BridgeNet2.ReferenceBridge("Replica_ReplicaSignal")             -- (replica_id, params...)
local rev_ReplicaSetParent = BridgeNet2.ReferenceBridge("Replica_ReplicaSetParent")       -- (replica_id, parent_replica_id)
local rev_ReplicaCreate = BridgeNet2.ReferenceBridge("Replica_ReplicaCreate")             -- (replica_id, {replica_data}) OR (top_replica_id, {creation_data}) or ({replica_package})
local rev_ReplicaDestroy = BridgeNet2.ReferenceBridge("Replica_ReplicaDestroy")           -- (replica_id)
...
local function ReplicaSetValue(content)
	local replica_id, path_array, value = table.unpack(content)
...
local function ReplicaSetValues(content)
	local replica_id, path_array, values = table.unpack(content)
	local replica = Replicas[replica_id]
	-- Getting path pointer and listener table:
	local pointer = replica.Data
	local listeners = replica._table_listeners
...
local function ReplicaArrayInsert(content) --> new_index
	
	local replica_id, path_array, value = table.unpack(content)
	print(replica_id, path_array, value, debug.traceback( ))
	local replica = Replicas[replica_id]
	-- Getting path pointer and listener table:
	local pointer = replica.Data
	local listeners = replica._table_listeners
...
local function ReplicaArraySet(content)
	local replica_id, path_array, index, value = table.unpack(content)
	local replica = Replicas[replica_id]
	-- Getting path pointer and listener table:
	local pointer = replica.Data
	local listeners = replica._table_listeners
...
local function ReplicaArrayRemove(content) --> removed_value
	local replica_id, path_array, index = table.unpack(content)
	local replica = Replicas[replica_id]
	-- Getting path pointer and listener table:
	local pointer = replica.Data
	local listeners = replica._table_listeners
...
function Replica:FireServer(...)
	rev_ReplicaSignal:Fire({self.Id, ...})
end
...
rev_ReplicaRequestData:Connect(function()
...
rev_ReplicaSetValue:Connect(ReplicaSetValue) -- (replica_id, {path}, value)

rev_ReplicaSetValues:Connect(ReplicaSetValues) -- (replica_id, {path}, {values})

rev_ReplicaArrayInsert:Connect(ReplicaArrayInsert) -- (replica_id, {path}, value)

rev_ReplicaArraySet:Connect(ReplicaArraySet) -- (replica_id, {path}, index, value)

rev_ReplicaArrayRemove:Connect(ReplicaArrayRemove) -- (replica_id, {path}, index)

rev_ReplicaWrite:Connect(function(content) -- (replica_id, func_id, {params})
	local replica_id, func_id = content[1], content[2]
	...
	replica._write_lib[func_id](replica, table.unpack(content))
	WriteFunctionFlag = false
	-- Signaling listeners:
	local listeners = replica._function_listeners[func_id]
	if listeners ~= nil then
		for _, listener in ipairs(listeners) do
			listener(table.unpack(content))
		end
	end
end)

-- Replica signals:
rev_ReplicaSignal:Connect(function(content) -- (replica_id, params...)
	local replica_id = content[1]
	table.remove(content,1)
	local replica = Replicas[replica_id]
	-- Signaling listeners:
...
-- Inheritance:
rev_ReplicaSetParent:Connect(function(content) -- (replica_id, parent_replica_id)
	local replica_id, parent_replica_id = table.unpack(content)
...
-- Replica creation:
rev_ReplicaCreate:Connect(function(content) -- (top_replica_id, {replica_data}) OR (top_replica_id, {creation_data}) or ({replica_package})
	--[[
	param1 description:
	...
	--]]
	local param1, param2 = table.unpack(content)
...
-- Replica destruction:
rev_ReplicaDestroy:Connect(function(content) -- (replica_id)
	local replica_id = content[1]
	local replica = Replicas[replica_id]
